...
def sign_armor_as_owner(ew3, wallet, args):
    print("When prompted, please sign transaction.")
    status = ew3.v0.submit_enable_module_signature(args.auth_address, wallet)
    if not status:
        bail("failed to submit enable module signature")


def enable_armor(ew3, wallet, args):
...
def addresses(ew3, wallet, args):
    armor_address, safe_address = ew3.v0.get_armor_and_safe_addresses(wallet.address)
    print(f"Armor address: {armor_address}")
    print(f"Safe address:  {safe_address}")


def create_whitelist(ew3, wallet, args):
    list_id = ew3.v0.create_draft_client_whitelist(args.addresses)
    print(f"Created draft client whitelist with ID {list_id}.")


def sign_whitelist(ew3, wallet, args):
    print("When prompted, please sign transaction.")
...
def get_whitelist(ew3, wallet, args):
    whitelist = ew3.v0.get_current_client_whitelist(is_draft=args.draft)
...
if __name__ == "__main__":
    ...
    if env_token:
        parser.add_argument(
            ...
            "--token",
            default=env_token,
            help="Refresh token for the Eulith API (defaults to EULITH_REFRESH_TOKEN env var)",
        )
    else:
        parser.add_argument(
            "-t",
    ...
    parser_sign_armor.add_argument("--auth-address", required=True, metavar="ADDR")
    ...
    if args.wallet_type == DUMMY_WALLET_TYPE:
        wallet = DummyWallet()
        ew3 = DummyEw3()
    else:
        if args.wallet_type == KMS_WALLET_TYPE:
            wallet = get_kms_wallet()
        elif args.wallet_type == LEDGER_WALLET_TYPE:
